...
?>
	...
	<tr id="cart_headings"><th></th><th align=left>Product</th><th align=left>Item Price</th><th align=left>Qty.</th><th align=left>Item Total</th><th></th></tr>
	<?php
	$subtotal = 0;
	...
	{	
		if($quantity > 0){
			...
			print '<p><tr>
					<td><a href="./item.php?item='.$item_id.'"><img border="0" width="60" src="./display_image.php?item_id='.$item_id.'" /></a></td>
					<td><small>'.$product['brand'].'</small><br /><b><a href="./item.php?item='.$item_id.'">'.$product['name'].'</a></b></td>
					<td>'.number_format($product['price'], 2).'</td>
					<td><form> <input type="text" id="qty'.$item_id.'" value="'.$quantity.'" size="2" ></form></td>
					<td>$<span id="item_'.$item_id.'_total">'.number_format($item_total, 2).'</span></td>
					<td>
						<p><form action="" method="post" onsubmit="return updateCartQuantity('.$item_id.', '.$product['quantity'].')">
							<input style="font-size: 70%" type="submit" value="Qty. Update" />
						</form></p>
						<form action="" method="post">
							<input type="hidden" name="id" value="'.$item_id.'" />
							<input type="hidden" name="submit_remove" value="true" />
							<input style="font-size: 70%" type="submit" value="remove&nbsp&nbsp&nbsp&nbsp&nbsp" />
						</form>
					</td>
					</tr></p>'."\n";
		}
